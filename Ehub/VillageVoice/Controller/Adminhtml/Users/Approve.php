        ...
        if ($isPost) {
            ...
            try {
                foreach ($selected_ids as $selected_id) {
                    ...
                    $customer->setCustomAttribute('is_vv_user', 1);
                    $customer->setCustomAttribute('request_for_vv_user', 0);
                    $customerModel->save($customer);
					
                    $connection->update($tableName, ['v_user_status' => '1'], ['v_user_id IN (?)' => $selected_id]);					
                }

                // Display success message
                $this->messageManager->addSuccess(__('Selected user accounts have been approved.'));
